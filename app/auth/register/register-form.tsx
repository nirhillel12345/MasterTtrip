...
export function RegisterForm({ nextPath }: Props) {
  const [state, formAction, pending] = useActionState<AuthFormState, FormData>(signUpWithPassword, null);
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        if (name.length < 2) {
          e.preventDefault();
          setClientError("נא להזין שם מלא (לפחות 2 תווים).");
          return;
        }
    ...
    >
      ...
      <label className="block text-right">
        ...
        />
        <span className="mt-1 block text-xs text-slate-500">לפחות 6 תווים</span>
      </label>
