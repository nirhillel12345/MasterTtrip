...
export default async function NewAttractionPage() {
  ...
  if (!user?.email) {
    redirect("/auth/login?error=" + encodeURIComponent("יש להתחבר כדי לפרסם אטרקציה") + "&next=/attractions/new");
  }
      ...
      <section className="mx-auto w-full max-w-3xl overflow-x-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/60 sm:p-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div className="text-right">
            <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">פרסום אטרקציה</h1>
            <p className="mt-1.5 text-sm text-slate-600">שתפו פעילות מעניינת עם הקהילה ותנו לכל המטיילים להצטרף.</p>
          </div>
          <Link
            href="/attractions"
