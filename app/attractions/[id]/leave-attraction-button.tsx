...
import { leaveAttraction } from "@/app/attractions/actions";

type Props = { attractionId: string };

export function LeaveAttractionButton({ attractionId }: Props) {
  ...
  async function onLeave() {
    setPending(true);
    setError(null);
    const res = await leaveAttraction(attractionId);
    setPending(false);
    if (!res.ok) {
      setError(res.error);
      return;
