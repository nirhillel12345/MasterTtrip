...
import { removeAttractionParticipant } from "@/app/attractions/actions";
...
export function RemoveAttractionParticipantButton({ attractionId, participantUserId }: Props) {
  ...
  async function onRemove() {
    setPending(true);
    setError(null);
    const res = await removeAttractionParticipant(attractionId, participantUserId);
    setPending(false);
    if (!res.ok) {
      setError(res.error);
      return;
